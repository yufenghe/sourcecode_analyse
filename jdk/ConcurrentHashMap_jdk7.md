...
```
到此，ConcurrentHashMap的jdk7代码主要的代码就分析完了，分段锁的使用使得容器提高了并发度，接下来会进行jdk8的代码分析，jdk8对ConcurrentHashMap进行了很大的改变，针对jdk7并发度segment数组长度固定，jdk8中少了一次寻址，数据结构增加了红黑树，以及链表与红黑树的转换，同时针对将Segment改为Node，使用cas和Synchronized替换了ReentrantLock。接下来会进行jdk8的代码分析。
